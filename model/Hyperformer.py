...
class STD_unit(nn.Module):
    def __init__(self, in_channels, out_channels, num_point=25, window_size=16, rel_reduction=8):  # r=4 or r=16
        super(STD_unit, self).__init__()
        self.rel_channels = in_channels // rel_reduction
        self.conv1 = nn.Conv1d(in_channels, self.rel_channels, 1)
        self.convs = nn.Linear(self.rel_channels*num_point, out_channels)
        self.conv2 = nn.Conv1d(in_channels, self.rel_channels, 1)
        self.convt = nn.Linear(self.rel_channels*window_size, out_channels)
    ...
    def forward(self, x):
        N = x.size(0)
        # temporal pooling
        x_s = self.conv1(x.mean(-2)).view(N, -1)
        x_s = self.convs(x_s)

        # spatial pooling
        x_t = self.conv2(x.mean(-1)).view(N, -1)
        x_t = self.convt(x_t)
        return x_s, x_t
...
class Model(nn.Module):
    def __init__(self, num_class=60, num_point=20, num_person=2, graph=None, graph_args=dict(), in_channels=3,
        ...
        self.std = STD_unit(24*num_of_heads, 256, num_point, 16)
        # standard ce loss
    ...
    def forward(self, x, y):
        ...
        _ , C, T, V = x.size()
        x_s, x_t = self.std(x)
        x_s = x_s.view(N, M, -1).mean(1)
        x_t = x_t.view(N, M, -1).mean(1)
