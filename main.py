    ...
    def load_model(self):
        ...
        if self.arg.phase == 'train':
            ...
            if 'ucla' in self.arg.work_dir:
                label_all = self.data_loader['train'].dataset.label * self.arg.train_feeder_args['repeat'] if self.arg.phase == 'train' else 0
            else:
                label_all = self.data_loader['train'].dataset.label if self.arg.phase == 'train' else 0
            assert len(label_all) == mem_size
            self.contrast_s = ContrastLoss(out_channels=256, T=self.arg.temperature, class_num=self.arg.model_args["num_class"], \
                                        mem_size=mem_size, label_all=label_all).cuda(output_device)
            self.contrast_t = ContrastLoss(out_channels=256, T=self.arg.temperature, class_num=self.arg.model_args["num_class"], \
                                        mem_size=mem_size, label_all=label_all).cuda(output_device)
    ...
    def train(self, epoch, save_model=False):
        ...
        for batch_idx, (data, label, index) in enumerate(process):
            ...
            with torch.cuda.amp.autocast(enabled=use_amp):
                ...
                output, z, x_s, x_t = self.model(data, F.one_hot(label, num_classes=self.model.num_class))

                loss = self.loss(output, label)

                if x_s is not None and x_t is not None:
                    contrast_loss = (self.contrast_s(x_s, label, index) + self.contrast_t(x_t, label, index)) / 2
                else:
        ...
        self.print_log(
            '\tMean training loss: {:.4f}. Mean contrast loss: {:.4f}. Mean training acc: {:.2f}%.'.format(np.mean(loss_value), np.mean(contrast_loss_value), np.mean(acc_value)*100))
        ...
        for ln in loader_name:
            ...
            for batch_idx, (data, label, index) in enumerate(process):
                ...
                with torch.no_grad():
                    ...
                    label = label.long().cuda(self.output_device)
                    # output, z = self.model(data, F.one_hot(label, num_classes=self.model.module.num_class))
                    output, z, _, _ = self.model(data, F.one_hot(label, num_classes=self.model.num_class))
                    if arg.ema:
